...
mongoimport --host ${MONGO_HOST:-mongo} --username ${MONGO_INITDB_ROOT_USERNAME} --password ${MONGO_INITDB_ROOT_PASSWORD} --db yummy-yams --collection events --authenticationDatabase=admin --file /usr/src/app/events.json --jsonArray